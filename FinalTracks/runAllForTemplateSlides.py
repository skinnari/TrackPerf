...
for PU in PUs:
	for truncation in truncationOptions:
		...
		if not os.path.isfile( inputFile+'.root' ): 
			...
			continue
		print '---> ',inputFile
                run ( 'L1TrackNtuplePlot("{inputFile}","{treeName}",0,0,0,false,false,3)'.format(inputFile=inputFile, treeName=treeName) )
 		print '---------> Muons'
 		run ( 'L1TrackNtuplePlot("{inputFile}","{treeName}",0,13,0,false,false,3)'.format(inputFile=inputFile, treeName=treeName) )
 		print '---------> Electrons'
 		run ( 'L1TrackNtuplePlot("{inputFile}","{treeName}",0,11,0,false,false,3)'.format(inputFile=inputFile, treeName=treeName) )

		print 'In jet'
		run ( 'L1TrackNtuplePlot("{inputFile}","{treeName}",1,0,0,false,false,3)'.format(inputFile=inputFile, treeName=treeName) )
		print 'In high pt jet'
		run ( 'L1TrackNtuplePlot("{inputFile}","{treeName}",2,0,0,false,false,3)'.format(inputFile=inputFile, treeName=treeName) )
		print 'In very high pt jet'
		run ( 'L1TrackNtuplePlot("{inputFile}","{treeName}",3,0,0,false,false,3)'.format(inputFile=inputFile, treeName=treeName) )
        ...
        for PU in PUs:
            for truncation in truncationOptions:
                ...
                if not os.path.isfile( inputFile+'.root' ): 
                    continue
                print '--------->',inputFile
                run ( 'L1TrackNtuplePlot("{inputFile}","{treeName}",0,{pdg},0,false,false,3)'.format(inputFile=inputFile,pdg=pdg, treeName=treeName) )

    print "Done"

 # Min bias
for truncation in truncationOptions:
      run ( 'L1TrackNtuplePlot("OnlyPU_PU200_{truncation}","{treeName}",0,0,0,false,false,3)'.format(truncation = truncation, treeName=treeName) )
